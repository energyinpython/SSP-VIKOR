...
# VIKOR ascertaining Strong Sustainability Paradigm (VASSP) method
class VASSP(MCDA_method):
    ...
    def __call__(self, matrix, weights, types, s_coeff = 0):
        ...
        >>> pref = vassp(matrix, weights, types, s_coeff = s_set)
        >>> rank = rank_preferences(pref, reverse = False) 
        """

        VASSP._verify_input_data(matrix, weights, types)
        return VASSP._vassp(self, matrix, weights, types, self.normalization_method, self.v, s_coeff)
    ...
    @staticmethod
    def _vassp(self, matrix, weights, types, normalization_method, v, s_coeff):
