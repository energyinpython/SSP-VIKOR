...
def main():
    ...
    iterations = 1000
    alts_ax = np.arange(2, 12, 2)

    # Alternatives
    alters = np.array([10, 20, 30, 40, 50])
    ...
    for i in range(iterations):
        ...
        for it, m in enumerate(alters):
            ...
            results_dict_rw['variable'].append(m)
    ...
    ax = sns.scatterplot(x="Alternatives", y="Correlation", data=df_final, hue="Methods", s = 50, alpha = 0.1, marker = 'o')
    ax.set_xticks(alts_ax)
    ax.set_xticklabels(alters, fontsize = 14)
    ...
    plt.tight_layout()
    plt.savefig('benchmarkable/bench_correlationsAlts.png')
    plt.savefig('benchmarkable/bench_correlationsAlts.eps')
    plt.savefig('benchmarkable/bench_correlationsAlts.pdf')
    plt.show()
